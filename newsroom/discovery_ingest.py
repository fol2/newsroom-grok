...
from newsroom.increment9.proving import SOURCE_URLS, assert_allowed_url, default_fetch
...
def load_official_rss_body() -> tuple[str, str, bytes]:
    ...
    if override:
        ...
        if len(body) > MAX_BODY_BYTES:
            raise ValueError("RSS body exceeds bound")
        return source_id, url, body
    status, body = default_fetch(url)
    if status != 200:
        raise ValueError(f"official RSS fetch failed: http-{status}")
    return source_id, url, body
