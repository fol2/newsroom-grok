...
def _drop_empty_v36_live_official_original_write_schema(
    connection: sqlite3.Connection,
) -> None:
    if int(connection.execute("PRAGMA user_version").fetchone()[0]) != 36:
        return
    from newsroom.authority.live_official_original_write_migrations import (
    ...
    if connection.execute(
        ...
        raise sqlite3.DatabaseError(
            "downgrade requires exact empty v36 original write schema"
        )
    for table in tables:
        if connection.execute(f'SELECT COUNT(*) FROM "{table}"').fetchone() != (0,):
            raise sqlite3.DatabaseError("v36 original write tables must be empty")
    guard = connection.execute(
        "SELECT sql FROM sqlite_master WHERE type='trigger' "
        "AND name='immutable_authority_migrations_delete'"
    ).fetchone()[0]
...
def _drop_empty_v35_live_official_evidence_package_schema(
    ...
    _drop_empty_v36_live_official_original_write_schema(connection)
